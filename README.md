...
│   └── demo2/                   # Demo app — device info + persistent login flow
...
> **The demo app** (`apps/demo2`) shows `@kafitra/lynx-device-info` and `@kafitra/lynx-async-storage` in action — device info card, persistent login session (survives app restarts via `@kafitra/lynx-storage`), and hot reload.
